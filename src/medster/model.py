...
# ---------------------------------------------------------------------------
# OptiQ inference path — bypasses Ollama HTTP, runs mlx_vlm directly
# ---------------------------------------------------------------------------
...
def call_opti_llm(
    ...
    Bypasses Ollama HTTP entirely — uses the already-loaded OptiQ model in
    memory. Same call signature as call_llm(); agent.py swaps them via OPTI_ALL_MODE.

    JSON reliability strategy (no Ollama grammar constraint available):
    - temperature=0 for all structured calls (near-deterministic at 35B)
    - explicit JSON example injected into every structured prompt
    - up to 2 retries with tightened instruction on parse failure
    - existing _parse_json_to_schema() handles think-tag stripping automatically
    """
    from medster.tools.analysis.primitives import _vision_generate

    # Merge system prompt into user prompt — mlx_vlm apply_chat_template
    # uses a single user turn; the model reads system context from prefix.
    ...
    if output_schema:
        ...
        # All retries failed — return empty tool call so agent can handle it
